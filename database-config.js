...
async function uploadDataToMongoFromJson(path = "./quickdb.json") {

    const driver = new MongoDriver(process.env.database_mongoURL)
    await driver.connect()

    const db = new QuickDB({ driver })
    const allJsonData = JSON.parse(readFileSync(path))
    await db.deleteAll()
    ...
    console.log("Done");
    await saveDataFromMongo()
    process.exit()
}

async function saveDataFromMongo(path = "./quickdb.json") {
    ...
    writeFileSync(path, JSON.stringify(await db.all()))
    console.log("File saved.");
    process.exit()
}

void saveDataFromMongo()
/**
 * @copyright
